...
Pノッチも、N・Bノッチと同じ10 tick線形応答と再遷移規則の対象とする。P3をCreateの
`baseAcceleration`に対応する中央ノッチとし、隣接Pノッチの倍率差を定数`ε_P = 1/5`とする。

```text
aTarget(Pn) = baseAcceleration × {1 + (n - 3) × ε_P}
```

| ノッチ | P1 | P2 | P3 | P4 | P5 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 倍率 | 0.6 | 0.8 | 1.0 | 1.2 | 1.4 |

Pノッチは実車の絶対加速度を固定値として移植するものではない。Createのユーザー設定
`baseAcceleration`への倍率であるため、設定を現実寄りまたはゲーム寄りへ変更しても、
P1〜P5の段階構造を保つ。速度依存補正はPhase 10の実走行調整まで導入しない。

### N（惰行）

Phase 6のNは速度を完全に維持する0加速度ではない。力行も常用制動も要求しない惰行として、
走行抵抗相当の一定減速を持つ。

```text
aTarget(N) = -baseAcceleration / 6
```

`1/6`はPhase 6の初期値であり、Phase 10の実走行データに基づく調整対象である。NはBノッチではない。
PまたはBからNへ移るときも、現在の`effectiveAcceleration`から上記目標へ10 tickで線形遷移する。
Nの定常到達後、Create境界ではその一回の呼出に限り、`finalTargetSpeed = 0`と
`accelerationMod = 1/6`を用いる。PまたはBからNへの10 tick遷移中は、実効加減速度の符号に
応じてCreate targetの向きを決め、遷移の途中で速度変化の向きを早期反転させてはならない。
後続tickのnative targetを保持・上書きしてはならない。

P/N/Bの自動選択、予測、ヒステリシス、B4/B5を中心とした通常制動の規則は
`docs/design/TARGET_SPEED_RESOLUTION.md`を正本とする。
