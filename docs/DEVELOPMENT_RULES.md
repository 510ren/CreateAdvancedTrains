...
9. Solの確認はJavaソースの静的確認とGradle buildまでとする。Minecraftの起動・操作、
   走行試験、ログ採取、実行時受入試験はユーザーが手動で行い、Solの完了条件に含めない。
...
Create 6.0.8の挙動、単位、ソースレベル受入条件、Gradle build、保護対象をMarkdownへ
記録する。実行時の確認が必要な場合はユーザー手動試験として分離する。Solは不整合や
